...
object StatismoLineIO {
  ...
  def readStatismoLineMeshModel(file: File, modelPath: String = "/"): Try[StatisticalLineMeshModel] = {
    ...
    val modelOrFailure = for {
      ...
      mesh <- representerName match {
        ...
        case _ =>
          h5file.readStringAttribute(s"$modelPath/representer/", "datasetType") match {
            case Success("Line_MESH") => readStandardMeshFromRepresenterGroup(h5file, modelPath)
            case Success(datasetType) => Failure(new Exception(s"can only read model of datasetType Line_MESH. Got $datasetType instead"))
      ...
      StatisticalLineMeshModel(mesh, meanDefVector, pcaVarianceVector, pcaBasis)
    }
  ...
  private def writeRepresenterStatismov090(h5file: HDF5File, group: Group, model: StatisticalLineMeshModel, modelPath: String): Try[Unit] = {

    val cellArray = model.referenceMesh.cells.map(_.ptId1.id) ++ model.referenceMesh.cells.map(_.ptId2.id)
    val pts = model.referenceMesh.pointSet.points.toIndexedSeq.par.map(p => (p.toArray(0).toDouble, p.toArray(1).toDouble))
    val pointArray = pts.map(_._1) ++ pts.map(_._2)
    ...
    for {
      ...
      _ <- h5file.writeStringAttribute(group.getFullName, "datasetType", "Line_MESH")

      _ <- h5file.writeNDArray[Int](s"$modelPath/representer/cells", NDArray(IndexedSeq(2, model.referenceMesh.cells.size), cellArray.toArray))
      _ <- h5file.writeNDArray[Float](s"$modelPath/representer/points", NDArray(IndexedSeq(2, model.referenceMesh.pointSet.points.size), pointArray.toArray.map(_.toFloat)))
    } yield Success(())
  }

  private def writeRepresenterStatismov081(h5file: HDF5File, group: Group, model: StatisticalLineMeshModel, modelPath: String): Try[Unit] = {

    // we simply store the reference into a vtk file and store the file (the binary data) into the representer

    def refAsByteArray(ref: LineMesh[_2D]): Try[Array[Byte]] = {
      val tmpfile = File.createTempFile("temp", ".vtk")
      tmpfile.deleteOnExit()
      for {
        _ <- MeshIO.writeLineMesh[_2D](ref, tmpfile)
