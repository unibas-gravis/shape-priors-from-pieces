...
object LineModelMetrics {
   ...
   *
   */
  def specificity(pcaModel: StatisticalLineMeshModel, data: Iterable[LineMesh[_2D]], nbSamples: Int)(
    implicit rng: Random
  ): Double = {
   ...
   */
  def generalization(pcaModel: StatisticalLineMeshModel, dc: DataLineCollection): Try[Double] = {
