...
import scalismo.common.{DiscreteField, NearestNeighborInterpolator2D, UnstructuredPointsDomain}
import scalismo.geometry.{EuclideanVector, EuclideanVector2D, Point, Point2D, _2D, _3D}
...
//    val meanTransformation = Transformation((pt : Point[_2D]) => pt + continuousMeanDeformationField(pt))
    ...
    val meanMesh = LineMesh[_2D](CreateUnstructuredPointsDomain2D.create(meanPoints), reference.topology)
    println("finish")
    meanMesh
  }
