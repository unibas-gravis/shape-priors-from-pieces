...
object RegistrationComparison {
  ...
  def evaluateReconstruction2GroundTruth(id: String, reconstruction: LineMesh2D, groundTruth: LineMesh2D): Unit = {
    val avgDist2Surf = LineMeshMetrics2D.avgDistance(reconstruction, groundTruth)

    val hausdorffDistance = LineMeshMetrics2D.hausdorffDistance(reconstruction, groundTruth)
  ...
  private def avgDistanceBoundaryAware(m1: LineMesh[_2D], m2: LineMesh[_2D]): (Double, Double) = {
    ...
    val dists = for (p <- pointsOnSample) yield {
      val pTarget = m2.pointSet.findClosestPoint(p).point
      (pTarget - p).norm
    }
