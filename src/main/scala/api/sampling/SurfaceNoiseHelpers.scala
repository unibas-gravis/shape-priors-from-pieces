...
object SurfaceNoiseHelpers {
  ...
  def surfaceNormalDependantNoise(
    ...
    */
    val firstTangentialDirection = EuclideanVector3D(0, 0, 1) //{
    ...
    MultivariateNormalDistribution(DenseVector.zeros[Double](3), Seq(
      ...
      (firstTangentialDirection.toBreezeVector, 0.00000000001)
    ))
  }
  ...
  def surfaceNormalDependantNoise2D(
    ...
    */
    ...
    MultivariateNormalDistribution(DenseVector.zeros[Double](2), Seq(
      (normalDirection.toBreezeVector, normalNoiseVariance),
      (tangentialVector.toBreezeVector, tangentialNoiseVariance),
    ))
  }

  //  def computeNormalFromPointCloud()
}